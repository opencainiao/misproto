package com.misproto.common.util.useragent;
...
public class UserAgentUtil {
	...
	public static boolean isLowBrowser(String browser, String version) {

		if (StringUtil.isEmpty(browser)) {
			return false;
		}
		if (browser.equals(BROWSER_OTHER)) {
			return true;
		}
	 ...
	 * 根据设备的用户使用量降序排列，这样对于大多数用户来说可以少判断几次即可拿到结果： <br>
	 * 操作系统:Windows > 苹果 > 安卓 > Linux > ... <br>
	...
	private static UserAgent judgeBrowser(String userAgent,
		...
		if (userAgent.contains("chrome")) {
			...
			return new UserAgent(BROWSER_CHROME, chromeVersion, platformType,
			...
			return new UserAgent(BROWSER_FIREFOX, ffVersion, platformType,
					platformSeries, platformVersion);
			...
			if (userAgent.contains("msie 10.0")) {// Internet Explorer 10
				return new UserAgent(BROWSER_IE, "10", platformType,
						platformSeries, platformVersion);
			} else if (userAgent.contains("msie 9.0")) {// Internet Explorer 9
				return new UserAgent(BROWSER_IE, "9", platformType,
						platformSeries, platformVersion);
			} else if (userAgent.contains("msie 8.0")) {// Internet Explorer 8
				return new UserAgent(BROWSER_IE, "8", platformType,
						platformSeries, platformVersion);
			} else if (userAgent.contains("msie 7.0")) {// Internet Explorer 7
				return new UserAgent(BROWSER_IE, "7", platformType,
						platformSeries, platformVersion);
			} else if (userAgent.contains("msie 6.0")) {// Internet Explorer 6
				return new UserAgent(BROWSER_IE, "6", platformType,
						platformSeries, platformVersion);
			}
		} else {// 暂时支持以上三个主流.其它浏览器,待续...
			return new UserAgent(BROWSER_OTHER, null, platformType,
					platformSeries, platformVersion);
		}
		return null;
	}
