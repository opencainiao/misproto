$().ready(function() {
	...
	$("#btn_save").bind("click", function() {
		...
		var url_to = $.getSitePath() + "/materialtypehome/add_supplier";
		
		var materialtype_supplier_id = $("#materialtype_supplier_id").val();
		if (materialtype_supplier_id != ""){
			//alert(materialtypedetailid);
			
			url_to = $.getSitePath() + "/materialtypehome/"+ materialtype_supplier_id +"/update_supplier";
		}
		...
		$.ajax({
	        ...
	        success: function(data){
				if (data['success'] == 'n'){
	        		if (data['brErrors']){
	        			$.showBRErrors(data['brErrors'],$("#materialtype_supplier_edit_div"));
	        		...
	        		// 刷新父页面的详细信息
	        		parent.reset_materailtype_supplier(content);
	        	}
	        },
